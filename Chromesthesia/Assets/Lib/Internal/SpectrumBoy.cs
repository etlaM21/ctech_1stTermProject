...
public class SpectrumBoy : MonoBehaviour {
	...
	public void updateRealTimeCanvas(float time){
		if(time > currentTime && currentIndex + 1 != spectrum.Count){
			...
			for(int i = 0; i < spectrals; i++){
				...
				button.GetComponent<RectTransform>().localScale = new Vector3(button.GetComponent<RectTransform>().localScale.x, spectrum[currentIndex].Item2[i], 1);
			}
		}
	}
